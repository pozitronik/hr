...
use app\models\SLA\SlaIgnoreCalendar;
use app\models\telegram\TelegramLC;
use app\models\users\Users;
use app\models\users\UsersFilters;
use app\models\users\UsersMessages;
use DateTime;
use Exception;
use Yii;
use yii\helpers\Url;
...
class Utils {
	 ...
	 * @return int
	 */
	public static function SummaryTime($dateStart, $dateEnd = false, $ignore_holidays = true, &$stats = []) {
	 ...
	 * @return int
	 */
	private static function getDayEnd($date) {
	...
	public static function isHoliday($date) {
		...
		if (in_array($dayOfWeek, [6, 7, 0])) return true;//PHP может считать воскресенье нулевым днём
		return SlaIgnoreCalendar::find()
				->where("TO_DAYS(`date`)=TO_DAYS(FROM_UNIXTIME($date))")
				->count() > 0;
	}
	 ...
	 * @return string
	 */
	public static function CypherFileName($filename) {
	...
	/**
	 * Генерируем URL, соответствующий фильтру
	 * @param $preset_id
	 * @return bool|string
	 */
	public static function FilterIdToUrl($preset_id) {
		/** @var UsersFilters $UF */
		$UF = UsersFilters::find()
			->where([
			...
			->one();
		if ($UF) {
			return self::FilterToUrl(json_decode($UF->filter, true));
		}
	 ...
	 * @return string
	 */
	public static function FilterToUrl($filter_array) {
		$n_filter = ['/'];
	 ...
	 * @return string
	 */
	public static function FormatDateToExport($date) {
	 ...
	 * @param string $title
	 */
	public static function log($some, $title = '') {
	 ...
	 */
	public static function fileLog($data, $title = false, $logName = 'debug.log', $format = self::PRINT_R) {
		$return_contents = '';
	 ...
	 */
	public function interval2seconds($interval) {
		$seconds = 0;
	 ...
	 */
	public static function append_node(SimpleXMLElement $to, SimpleXMLElement $from) {
	 ...
	 */
	public static function DecToPos($N, $alphabet) {
		$q = strlen($alphabet);
		$ret = '';
		while (true) {
			$i = $N % $q;
	...
	/**
	 * Пишет админу в чат + телеграм: используется для внутреннего оповещения об ошибках
	 * @param $prayer
	 * @throws Throwable
	 */
	public static function PrayToGod($prayer) {
		UsersMessages::LogMessage(1, 1, $prayer, false);
		if (Users::findModel(284)->telegram_chat_id) TelegramLC::SendMessage(Users::findModel(1)->telegram_chat_id, $prayer);
	}
	 ...
	 * @return bool
	 */
	public static function CreateDirIfNotExisted($path, $mode = 0777) {
		if (file_exists($path)) {
	 ...
	 * @throws Throwable
	 */
	public static function GetValueAsArray($array, $key, $default = []) {
	 ...
	 * @throws Throwable
	 */
	public static function MakeLike($param) {
	 ...
	 * @throws Throwable
	 */
	public static function IdSearchCondition($IdValue, $table = '') {
